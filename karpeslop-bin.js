 ...
 */
export function isExcludedPath(filePath, rootDir, allowOutsideRoot = false) {
  const relativePath = path.relative(rootDir, filePath).replace(/\\/g, '/');
  const isOutsideRoot = relativePath.startsWith('..');
  const pathToMatch = allowOutsideRoot && isOutsideRoot ? path.resolve(filePath).replace(/\\/g, '/') : relativePath;
...
class AISlopDetector {
  ...
  manifestFilenames = ['package.json', 'package-lock.json'];
  ...
  constructor(rootDir, targetPaths) {
    ...
    if (targetPaths && targetPaths.length > 0) {
      this.targetPaths = targetPaths.map(p => path.isAbsolute(p) ? p : path.resolve(this.rootDir, p));
    }
  }
...
export function splitCliArgs(args) {
